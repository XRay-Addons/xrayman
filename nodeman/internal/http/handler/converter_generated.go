...
func ConvertNewNodeResult(source *models.NewNodeResult) *gen.NewNodeResponse {
	...
	if source != nil {
		...
		apiNewNodeResponse.AccessSecret = (*source).AccessSecret
...
func modelsNodeConfigToApiNodeConfig(source models.NodeConfig) gen.NodeConfig {
	...
	apiNodeConfig.ID = ConvertNodeID(source.ID)
...
func modelsNodeConnectionInfoToApiNodeConnectionInfo(source models.NodeConnectionInfo) gen.NodeConnectionInfo {
	...
	if source.AccessKey != nil {
		apiNodeConnectionInfo.AccessKey = make([]uint8, len(source.AccessKey))
		for i := 0; i < len(source.AccessKey); i++ {
			apiNodeConnectionInfo.AccessKey[i] = source.AccessKey[i]
		}
	}
