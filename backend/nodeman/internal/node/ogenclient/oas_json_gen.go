...
// Encode implements json.Marshaler.
func (s *Performance) Encode(e *jx.Encoder) {
	e.ObjStart()
	s.encodeFields(e)
	e.ObjEnd()
}
...
// encodeFields encodes fields.
func (s *Performance) encodeFields(e *jx.Encoder) {
	{
		e.FieldStart("CpuLoad")
		e.Float32(s.CpuLoad)
	}
	{
		e.FieldStart("RamLoad")
		e.Float32(s.RamLoad)
	}
	{
		e.FieldStart("MemLoad")
		e.Float32(s.MemLoad)
	}
	{
		e.FieldStart("OpenConnections")
		e.Int32(s.OpenConnections)
	}
}

var jsonFieldsNameOfPerformance = [4]string{
	0: "CpuLoad",
	1: "RamLoad",
	2: "MemLoad",
	3: "OpenConnections",
...
// Decode decodes Performance from json.
func (s *Performance) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode Performance to nil")
	}
	var requiredBitSet [1]uint8
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		case "CpuLoad":
			...
			if err := func() error {
				...
				return errors.Wrap(err, "decode field \"CpuLoad\"")
			}
		case "RamLoad":
			requiredBitSet[0] |= 1 << 1
			if err := func() error {
				...
				return errors.Wrap(err, "decode field \"RamLoad\"")
			}
		case "MemLoad":
			requiredBitSet[0] |= 1 << 2
			if err := func() error {
				...
				return errors.Wrap(err, "decode field \"MemLoad\"")
			}
		case "OpenConnections":
			requiredBitSet[0] |= 1 << 3
			if err := func() error {
				v, err := d.Int32()
				s.OpenConnections = int32(v)
				if err != nil {
					return err
				}
				return nil
			}(); err != nil {
				return errors.Wrap(err, "decode field \"OpenConnections\"")
			}
		default:
			return d.Skip()
		}
		...
		return errors.Wrap(err, "decode Performance")
	}
	// Validate required fields.
	var failures []validate.FieldError
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfPerformance) {
					name = jsonFieldsNameOfPerformance[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
...
// MarshalJSON implements stdjson.Marshaler.
func (s *Performance) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	s.Encode(&e)
	return e.Bytes(), nil
}
...
// UnmarshalJSON implements stdjson.Unmarshaler.
func (s *Performance) UnmarshalJSON(data []byte) error {
	d := jx.DecodeBytes(data)
	return s.Decode(d)
}
...
var jsonFieldsNameOfStartResponse = [2]string{
...
var jsonFieldsNameOfStatsResponse = [2]string{
	0: "users",
	1: "performance",
}
	...
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfStatsResponse) {
					name = jsonFieldsNameOfStatsResponse[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
...
var jsonFieldsNameOfUserStat = [3]string{
	0: "ID",
	1: "Uplink",
	2: "Downlink",
}
