...
С нодами менеджер взаимодействует односторонне, вызывая их методы.
Авторизация - JWT, шифрование - TLS с CertPinning. AccessKey ноды представляет собой `jwt secret + hash(pubKey)`
...
// присвоить ей id
POST /node/new {endpoint, accessKey}
...
**sync_status** - статус пользователей на нодах - добавлены/не добавлены
- NodeID
- UserID
- CurrentStatus
...
Все сценарии выглядят так: мы получаем входящий запрос - удалить ноду, добавить пользователя, включить/выключить пользователя, и изменяем записи в таблицах `nodes` (кроме свойства `nodes::CurrentStatus`) или `users`.

Затем запускаем механизм синхронизации состояния нод и хранилища (меняем nodes::CurrentStatus и sync_status::CurrentStatus)

### Добавление новой ноды
1. запустить ноду - в консоль и в файл будет выведен `access_key` (он будет сгенерирован или взят с предыдущих запусков - он хранится в файле)
2. вызвать у node manager-а `/node/new {node_endpoint, access_key}`.
manager создаст ID, добавит ноду в базу данных/
...
3. менеджер отмечает в базе данных состояние ноды как неизвестное.

4. менеджер делает запрос к ноде `node_endpoint/start {users}`, используя `node_key`. 

4. делаем запрос к базе данных, отмечаем там ноду как запущенную (если запрос был успешен), меняем состояние пользователей на ноде в таблице `sync_status`

худшее, что может быть - запрос к базе данных из 4 пункта не будет успешен, тогда состояние ноды в базе навечно останется там как неизвестное (не навечно, а до следующей успешной попытки достучаться до ноды и бд).
...
5. делаем запрос к базе данных, отмечаем там ноду как остановленную (если запрос был успешен).

худшее, что может быть - запрос к базе данных из 4 пункта не будет успешен, тогда состояние ноды в базе навечно останется там как неактуальное (не навечно, а до следующей успешной попытки достучаться до ноды и бд).
...
3. менеджер вызывает у каждой запущенной ноды метод `/users/edit` в который передает пользователей из `sync_status` для данной ноды
