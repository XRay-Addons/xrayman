...
import (
	...
	"github.com/XRay-Addons/xrayman/node/internal/cmd"
...
func createService(domain, plistLocation string) error {
	_, _, err := cmd.Run(launchctl, []string{
...
func removeService(domain, service string) error {
	_, stderr, err := cmd.Run(launchctl, []string{
...
func getServiceStatus(domain, service string) (models.ServiceStatus, error) {
	stdout, _, err := cmd.Run(launchctl, []string{
		"print", filepath.Join(domain, service),
	})

	if err != nil {
		return models.ServiceStopped, fmt.Errorf(
			"%w: print: %v", errdefs.ErrService, err)
	}

	statusStr := extractStatusString(stdout)
	switch statusStr {
	case "running":
